...
def convert(input_path, output_path, title=None):
    ...
    # Unpack carousel blocks into sequential slide figures
    def unpack_carousel(match):
    ...
    md_text = re.sub(r'````carousel\s*\n(.*?)\n````', unpack_carousel, md_text, flags=re.DOTALL)
    md_text = re.sub(r'```carousel\s*\n(.*?)\n```', unpack_carousel, md_text, flags=re.DOTALL)
            ...
            if img_tag:
                ...
                if os.path.exists(src):
                    ...
                    try:
                        ...
                        with PILImage.open(src) as pimg:
                            pw, ph = pimg.size
                        if pw < 600:
                            calc_w = min(4.0, max(2.6, pw / 110.0))
                        else:
                            calc_w = 6.0
                        run.add_picture(src, width=Inches(calc_w))
                    except Exception as img_err:
                        try:
                            run.add_picture(src, width=Inches(5.5))
                        except Exception as e2:
                            print(f"Warning: Could not insert image {src}: {e2}")
                    alt = img_tag.get('alt')
                    if alt:
